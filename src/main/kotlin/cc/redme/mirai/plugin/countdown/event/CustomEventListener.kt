...
internal object CustomEventListener: CoroutineScope by PluginMain.childScope("CustomEventListener") {
    ...
    fun subscribe(){
        ...
        globalEventChannel().subscribeAlways<AddCountdownEvent> {
            ...
            message.sender.sendMessage("是否开启倒计时提醒? 0: 是 1: 否\n请回复 0 或 1 ")
            ...
            message.sender.sendMessage("请设置倒计时的回复格式, 回复 0 来使用默认值(距离{name}还有{time})")
            message.selectMessagesUnit {
                "0"{
            ...
            message.sender.sendMessage(CountdownTasker.addCountdown(name, time, pattern, target, notify, notifyInterval))
        }
    }
